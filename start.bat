...
call :echo_info "Downloading %filename%..."
...
if %errorlevel% neq 0 (
    call :echo_error "Failed to download %filename%"
    exit /b 1
)

call :echo_success "Downloaded %filename%"
exit /b 0
...
call :echo_info "Checking MongoDB installation..."

REM Check if mongod is in PATH
where mongod >nul 2>&1
if %errorlevel% equ 0 (
    call :echo_success "MongoDB is already installed (in PATH)"
    exit /b 0
)

REM Check if MongoDB is installed in default directory
if exist "%MONGODB_INSTALL_DIR%\mongod.exe" (
    call :echo_success "MongoDB is already installed at %MONGODB_INSTALL_DIR%"
    REM Add to PATH
    setx PATH "%MONGODB_INSTALL_DIR%;%PATH%" >nul 2>&1
    exit /b 0
)

call :echo_warning "MongoDB is not installed"
exit /b 1

REM Install MongoDB for Windows
:install_mongodb
call :echo_warning "MongoDB not found. Installing MongoDB Community Server..."
...
if %errorlevel% neq 0 (
    call :echo_error "Failed to download MongoDB"
    exit /b 1
)

REM Install MongoDB silently
call :echo_info "Installing MongoDB (this may take a few minutes)..."
msiexec /qb /i "%MONGO_MSI%" INSTALLLOCATION="%ProgramFiles%\MongoDB\Server\6.0\" ADDLOCAL="all" /norestart

if %errorlevel% neq 0 (
    call :echo_error "Failed to install MongoDB"
    del "%MONGO_MSI%" >nul 2>&1
    exit /b 1
)

REM Add MongoDB to PATH
call :echo_info "Adding MongoDB to PATH..."
setx PATH "%MONGODB_INSTALL_DIR%;%PATH%" >nul 2>&1

REM Clean up
del "%MONGO_MSI%" >nul 2>&1

call :echo_success "MongoDB installed successfully!"
call :echo_info "MongoDB installed at: %ProgramFiles%\MongoDB\Server\6.0\"
...
call :echo_info "Checking Tesseract OCR installation..."

REM Check if tesseract is in PATH
where tesseract >nul 2>&1
if %errorlevel% equ 0 (
    call :echo_success "Tesseract OCR is already installed (in PATH)"
    exit /b 0
)

REM Check if Tesseract is installed in default directory
if exist "%TESSERACT_INSTALL_DIR%\tesseract.exe" (
    call :echo_success "Tesseract OCR is already installed at %TESSERACT_INSTALL_DIR%"
    REM Add to PATH
    setx PATH "%TESSERACT_INSTALL_DIR%;%PATH%" >nul 2>&1
    exit /b 0
)

call :echo_warning "Tesseract OCR is not installed"
exit /b 1

REM Install Tesseract OCR for Windows
:install_tesseract
call :echo_warning "Tesseract OCR not found. Installing Tesseract OCR..."
...
if %errorlevel% neq 0 (
    call :echo_error "Failed to download Tesseract OCR"
    exit /b 1
)

REM Install Tesseract silently
call :echo_info "Installing Tesseract OCR..."
"%TESSERACT_INSTALLER%" /S /D="%TESSERACT_INSTALL_DIR%"

if %errorlevel% neq 0 (
    call :echo_error "Failed to install Tesseract OCR"
    del "%TESSERACT_INSTALLER%" >nul 2>&1
    exit /b 1
)

REM Add Tesseract to PATH
call :echo_info "Adding Tesseract OCR to PATH..."
setx PATH "%TESSERACT_INSTALL_DIR%;%PATH%" >nul 2>&1

REM Clean up
del "%TESSERACT_INSTALLER%" >nul 2>&1

call :echo_success "Tesseract OCR installed successfully!"
call :echo_info "Tesseract OCR installed at: %TESSERACT_INSTALL_DIR%"
...
call :echo_info "Note: On Windows, Calibri and Caladea fonts are typically available by default."
call :echo_info "If you need to install additional fonts, please install them manually."
call :echo_info "Linux font packages (ttf-mscorefonts-installer, fonts-crosextra-*) are not needed on Windows."

call :echo_success "Font check passed!"
goto :eof
...
echo.
echo =============================================================================
echo Single Command Deployment Script for Windows
echo =============================================================================
echo.

call :echo_status Checking and installing prerequisites...

REM Check for Node.js
where node >nul 2>&1
if %errorlevel% neq 0 (
    call :echo_error Node.js is not installed. Please install Node.js first.
    call :echo_info Download from: https://nodejs.org/
    exit /b 1
)
call :echo_success Node.js is installed

REM Check for Python
where python >nul 2>&1
if %errorlevel% neq 0 (
    call :echo_error Python is not installed. Please install Python 3 first.
    call :echo_info Download from: https://python.org/
    exit /b 1
)
call :echo_success Python is installed

REM Check for curl (needed for health checks)
where curl >nul 2>&1
if %errorlevel% neq 0 (
    call :echo_warning curl is not installed. Some health checks may not work.
)
...
call :echo_status All prerequisites are installed!

REM =============================================================================
REM Step 2: Start MongoDB
REM =============================================================================
:start_mongodb
call :echo_status Starting MongoDB...
...
if %errorlevel% equ 0 (
    call :echo_warning MongoDB is already running. Skipping MongoDB start.
    for /f "tokens=2" %%a in ('tasklist /FI "IMAGENAME eq mongod.exe" /NH') do set MONGO_PID=%%a
    goto :start_backend
)

REM Start MongoDB in background
call :echo_info Starting MongoDB on port 27017...
start /B mongod --dbpath "%MONGO_DB_PATH%" --bind_ip 127.0.0.1 --port 27017

REM Wait for MongoDB to be ready
call :echo_info Waiting for MongoDB to be ready...
timeout /t 5 /nobreak >nul

REM Verify MongoDB is running
tasklist /FI "IMAGENAME eq mongod.exe" 2>nul | findstr /I mongod.exe >nul
if %errorlevel% neq 0 (
    call :echo_error Failed to start MongoDB. Check logs/mongodb.log for details.
    exit /b 1
)

for /f "tokens=2" %%a in ('tasklist /FI "IMAGENAME eq mongod.exe" /NH') do set MONGO_PID=%%a
call :echo_status MongoDB started successfully (PID: %MONGO_PID%)

REM =============================================================================
REM Step 3: Start Backend
REM =============================================================================
:start_backend
call :echo_status Starting Backend...

cd /d "%BACKEND_DIR%"

REM Check if virtual environment exists
if not exist "%VENV_DIR%" (
    call :echo_info Creating Python virtual environment...
    python -m venv "%VENV_DIR%"
)

REM Install dependencies
call :echo_info Installing backend dependencies...
call "%VENV_DIR%\Scripts\pip" install -q -r requirements.txt 2>nul

REM Start backend in background
call :echo_info Starting backend server on port %BACKEND_PORT%...
start /B cmd /c "%VENV_DIR%\Scripts\python -m uvicorn server:app --host 127.0.0.1 --port %BACKEND_PORT%"

set BACKEND_PID=last
call :echo_status Backend started (checking status...)

REM Wait for backend to be ready
call :echo_info Waiting for backend to be ready...
timeout /t 5 /nobreak >nul

REM Verify backend is running
curl -s http://127.0.0.1:%BACKEND_PORT%/api >nul 2>&1
if %errorlevel% neq 0 (
    call :echo_warning Backend may still be starting, continuing...
)

call :echo_status Backend started successfully

REM =============================================================================
REM Step 4: Start Frontend
REM =============================================================================
:start_frontend
call :echo_status Starting Frontend...

cd /d "%FRONTEND_DIR%"

REM Check if node_modules exists
if not exist "node_modules" (
    call :echo_info Installing frontend dependencies...
...
call :echo_info Starting frontend server on port %FRONTEND_PORT%...
start cmd /c "npm start"

call :echo_status Frontend starting...
